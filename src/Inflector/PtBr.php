...
class PtBr extends Inflector 
{
	...
	public static function pluralize($word)
	{
		...
		if ( ! empty(self::$uninflected[$transliterated]) ) {
			$formatted = $word;
		}

		if( $formatted == '' ){
			if ( ! empty(self::$irregular[$word]) ) {
				$formatted = self::$irregular[$word];
			}
		}

		if( $formatted == '' ){
			if ( ! empty(self::$irregular[$transliterated]) ) {
				$formatted = self::$irregular[$transliterated];
			}
		}
		...
		if( ! empty(self::$formatted[$formatted]) ){
			$formatted = self::$formatted[$formatted];
		}
	...
	public static function singularize($word)
	{
		...
		if( $formatted == '' ){
			if ( in_array($word, self::$irregular) ) {
				$formatted = array_search($word, self::$irregular);
			}
		}

		if( $formatted == '' ){
			if ( in_array($transliterated, self::$irregular) ) {
				$formatted = array_search($transliterated, self::$irregular);
			}
		}
